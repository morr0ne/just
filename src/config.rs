use {
  super::*,
  clap::{builder::EnumValueParser, Arg, ArgAction, ArgGroup, ArgMatches, Command},
  clap_complete::shells::Shell,
};
...
mod arg {
  ...
  pub(crate) const DUMP_FORMAT_VALUES: [&str; 2] = [DUMP_FORMAT_JUST, DUMP_FORMAT_JSON];
}

impl Config {
  pub(crate) fn command() -> Command {
    let command = clap::command!()
      // TODO .setting(AppSettings::ColoredHelp)
      // TODO .setting(AppSettings::TrailingVarArg)
      .arg(
        Arg::new(arg::CHECK)
          .long("check")
          .requires(cmd::FORMAT)
      ...
      .arg(Arg::new(arg::YES).long("yes").help("Automatically confirm all recipes."))
      .arg(
        Arg::new(arg::DRY_RUN)
          .short('n')
          .long("dry-run")
      ...
      .arg(
        Arg::new(arg::DUMP_FORMAT)
          .long("dump-format")
          .num_args(1)
          .value_parser(arg::DUMP_FORMAT_VALUES)
    ...
    if cfg!(feature = "help4help2man") {
      command.version(env!("CARGO_PKG_VERSION")).about(concat!(
        "- Please see ",
  ...
  fn dump_format_from_matches(matches: &mut ArgMatches) -> ConfigResult<DumpFormat> {
    let value = matches
      .remove_one::<String>(arg::DUMP_FORMAT)
      .ok_or_else(|| ConfigError::Internal {
  ...
  pub(crate) fn from_matches(matches: &mut ArgMatches) -> ConfigResult<Self> {
    let invocation_directory = env::current_dir().context(config_error::CurrentDirContext)?;

    let verbosity = if matches.contains_id(arg::QUIET) {
      Verbosity::Quiet
    } else {
    ...
    if let Some(occurrences) = matches.remove_occurrences::<String>(arg::SET) {
      for mut occurence in occurrences {
        overrides.insert(
          occurence.next().unwrap().to_owned(),
          occurence.next().unwrap().to_owned(),
        );
      }
    }
    ...
    let positional =
      Positional::from_values(matches.remove_many(arg::ARGUMENTS).map(|values| values));
    ...
    let search_config = {
      ...
      let working_directory = matches.remove_one::<PathBuf>(arg::WORKING_DIRECTORY);
    ...
    let subcommand = if matches.contains_id(cmd::CHANGELOG) {
      Subcommand::Changelog
    } else if matches.contains_id(cmd::CHOOSE) {
      Subcommand::Choose {
        chooser: matches.remove_one(arg::CHOOSER).map(str::to_owned),
        overrides,
      }
    } else if let Some(values) = matches.remove_many(cmd::COMMAND) {
    ...
    } else if let Some(shell) = matches.remove_one::<String>(cmd::COMPLETIONS) {
      Subcommand::Completions {
        shell: shell.to_owned(),
      }
    } else if matches.contains_id(cmd::EDIT) {
      Subcommand::Edit
    } else if matches.contains_id(cmd::SUMMARY) {
      Subcommand::Summary
    } else if matches.contains_id(cmd::DUMP) {
      Subcommand::Dump
    } else if matches.contains_id(cmd::FORMAT) {
      Subcommand::Format
    } else if matches.contains_id(cmd::INIT) {
      Subcommand::Init
    } else if matches.contains_id(cmd::LIST) {
      Subcommand::List
    } else if let Some(name) = matches.remove_one::<String>(cmd::SHOW) {
      Subcommand::Show {
        name: name.to_owned(),
      }
    } else if matches.contains_id(cmd::EVALUATE) {
    ...
    } else if matches.contains_id(cmd::VARIABLES) {
    ...
    let shell_args = if matches
      .remove_occurrences::<String>(arg::SHELL_ARG)
      .is_some()
      || matches
        .remove_occurrences::<String>(arg::CLEAR_SHELL_ARGS)
        .is_some()
    {
      Some(
        matches
          .remove_many(arg::SHELL_ARG)
    ...
    let unstable = matches.contains_id(arg::UNSTABLE)
      || env::var_os("JUST_UNSTABLE")
        .map(|val| !(val == "false" || val == "0" || val.is_empty()))
        .unwrap_or_default();

    Ok(Self {
      check: matches.contains_id(arg::CHECK),
      color,
      command_color,
      dotenv_filename: matches.remove_one(arg::DOTENV_FILENAME).map(str::to_owned),
      dotenv_path: matches
        .remove_one::<PathBuf>(arg::DOTENV_PATH)
        .map(PathBuf::from),
      dry_run: matches.contains_id(arg::DRY_RUN),
      dump_format: Self::dump_format_from_matches(matches)?,
      highlight: !matches.contains_id(arg::NO_HIGHLIGHT),
      ...
      list_prefix: matches
        ...
        .unwrap_or("    ")
        .to_owned(),
      load_dotenv: !matches.contains_id(arg::NO_DOTENV),
      no_dependencies: matches.contains_id(arg::NO_DEPS),
      search_config,
      shell: matches.remove_one(arg::SHELL).map(str::to_owned),
      shell_args,
      shell_command: matches.contains_id(arg::SHELL_COMMAND),
      subcommand,
      unsorted: matches.contains_id(arg::UNSORTED),
      unstable,
      verbosity,
      yes: matches.contains_id(arg::YES),
    })
  }
...
mod tests {
  ...
  fn test(arguments: &[&str], want: Config) {
    ...
    let have = Config::from_matches(&mut matches).expect("config parsing failed");
    assert_eq!(have, want);
  }
  ...
  macro_rules! error {
    ...
    };
    {
      ...
      fn $name() {
        ...
        match Config::from_matches(&mut matches).expect_err("config parsing succeeded") {
          $error => { $($check)? }
